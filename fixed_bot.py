...
@bot.command(name="pvbps", aliases=["gs.pvbps"])
async def pvbps(ctx):
    """" Get Plants Vs Brainrots Private Server link"""
    embed = discord.Embed(
        title="Plants Vs Brainrots Private Server",
        color=0xFFC916,
        timestamp=discord.utils.utcnow()
    )
    embed.add_field(
        name="Private Server Link:",
        value="https://www.roblox.com/share?code=4010ffa1aaf7c74493ad8fed713e83df&type=Server",
        inline=False
    )
    embed.set_footer(text="🔥 Game Services")
    await ctx.send(embed=embed)
...
async def axo_command(ctx):
    """Handle axo command"""
    await ctx.send("🌮 TACO TUESDAY WILL BE AT **6 PM EST**\n**YOUR TIME:** <t:1758664800:f>")
